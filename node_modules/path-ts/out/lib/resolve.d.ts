 ...
 */
export type ResolvePathString<S extends string, Root extends string = "/", Ss extends string[] = []> = S extends `/${string}` ? Join<[S, ...Ss], "/"> : `${Root}/${Join<[S, ...Ss], "/">}`;
/**
 * Type-utility for resolving a relative path-like string to an absolute path.
 *
 ...
 */
export type ResolvePathBuilderLike<T extends PathBuilderLike = PathBuilderLike, Root extends string = "/", Pn extends string[] = []> = T extends PathBuilder<infer S> ? ResolvePathString<S, Root, Pn> : T extends string ? ResolvePathString<T, Root, Pn> : never;
/**
 ...
 */
export declare function resolvePathBuilder<T extends PathBuilderLike, Pn extends string[]>(pathSegment1?: T, ...pathSegmentN: Pn): PathBuilder<ResolvePathBuilderLike<T, "/{$CWD}", Pn>>;
export interface PathBuilderResolver<RuntimeRootAlias extends string = "~"> {
    (): PathBuilder<RuntimeRootAlias>;
    <T extends PathBuilderLike, Pn extends string[] = []>(pathSegment1?: T, ...pathSegmentN: Pn): PathBuilder<ResolvePathBuilderLike<T, RuntimeRootAlias, Pn>>;
}
/**
 ...
 */
export declare function createPathBuilderResolver<RuntimeRootAlias extends string = "~">(absoluteRuntimeRoot: string): PathBuilderResolver<RuntimeRootAlias>;
